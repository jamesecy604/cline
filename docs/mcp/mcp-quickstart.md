...
Think of MCP servers as special helpers that give mayai extra powers! They let mayai do cool things like fetch web pages or work with your files.
...
1. From the mayai extension, click the `MCP Server` tab
...
1. mayai will detect the change automatically
2. The MCP installer will be downloaded and installed
3. mayai will start the MCP installer
4. You'll see the server status in mayai's MCP settings UI:
...
Now that you have the MCP installer, you can ask mayai to add more servers from:
...
For example, you can ask mayai to install the `mcp-server-fetch` package found on the Python Package Index:
...
That's it! 🎉 You've just given mayai some awesome new abilities!
...
Now for some good news. While not perfect, you can get mayai to do this for you fairly reliably for subsequent server install. Add the following to your "Custom Instructions" in the mayai settings (top-right toolbar button):

> When installing MCP servers and editing the mayai_mcp_settings.json, if the server requires use of `npx` as the command, you must copy the "env" entry from the "mcp-installer" entry and add it to the new entry. This is vital to getting the server to work properly when in use.
...
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that mayai is seeing.
