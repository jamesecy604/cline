...
import { BrowserAction, BrowserActionResult, mayaiMessage, mayaiSayBrowserAction } from "../../../../src/shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: mayaiMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: mayaiMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: mayaiMessage[] = []
		let nextActionMessages: mayaiMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div
				...
				<span style={{ fontWeight: "bold" }}>
					<>{isAutoApproved ? "mayai is using the browser:" : "mayai wants to use the browser:"}</>
				</span>
			</div>
			<div
				style={{
