...
describe("Extension Tests", function () {
	...
	it("should activate extension successfully", async () => {
		// Get the extension
		const extension = vscode.extensions.getExtension("saoudrizwan.mayai")
	...
	it("should open sidebar view", async () => {
		...
		await vscode.commands.executeCommand("mayai.plusButtonClicked")
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("mayai.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("mayai.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("mayai")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("mayai.disableBrowserTool", undefined, true)
	})
})
