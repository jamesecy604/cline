...
import { mayaiIgnoreController, LOCK_TEXT_SYMBOL } from "./ignore/mayaiIgnoreController"
import { parseMentions } from "./mentions"
import { formatResponse } from "./prompts/responses"
import { addUserInstructions, SYSTEM_PROMPT } from "./prompts/system"
import { getNextTruncationRange, getTruncatedMessages } from "./sliding-window"
import { mayaiProvider, GlobalFileNames } from "./webview/mayaiProvider"
...
export class mayai {
	...
	private async getSavedmayaiMessages(): Promise<mayaiMessage[]> {
	...
	private async savemayaiMessages() {
		try {
			const taskDir = await this.ensureTaskDirectoryExists()
			const filePath = path.join(taskDir, GlobalFileNames.uiMessages)
			await fs.writeFile(filePath, JSON.stringify(this.mayaiMessages))
	...
	async restoreCheckpoint(messageTs: number, restoreType: mayaiCheckpointRestore) {
	...
	private async resumeTaskFromHistory() {
		...
		const modifiedmayaiMessages = await this.getSavedmayaiMessages()
		...
		this.mayaiMessages = await this.getSavedmayaiMessages()

		// Now present the mayai messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldnt be initialized when opening a old task, and it was because we were waiting for resume)
		...
		// const lastmayaiMessage = this.mayaiMessages[lastmayaiMessageIndex]
		// could be a completion result with a command
		// const secondLastmayaiMessage = this.mayaiMessages
		// 	.slice()
		// 	.reverse()
		// 	.find(
		// 		(m, index) =>
		// 			index !== lastmayaiMessageIndex && !(m.ask === "resume_task" || m.ask === "resume_completed_task")
		// 	)
		// (lastmayaiMessage?.ask === "command" && secondLastmayaiMessage?.ask === "completion_result")
	...
	private async initiateTaskLoop(userContent: UserContent, isNewTask: boolean): Promise<void> {
		...
		while (!this.abort) {
			const didEndLoop = await this.recursivelyMakemayaiRequests(nextUserContent, includeFileDetails, isNewTask)
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		if (await fileExistsAtPath(mayaiRulesFilePath)) {
			try {
				...
				if (ruleFileContent) {
					mayaiRulesFileInstructions = `# .mayairules\n\nThe following is provided by a root-level .mayairules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${ruleFileContent}`
				}
			} catch {
		...
		if (mayaiIgnoreContent) {
			mayaiIgnoreInstructions = `# .mayaiignore\n\n(The following is provided by a root-level .mayaiignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${mayaiIgnoreContent}\n.mayaiignore`
		}

		if (settingsCustomInstructions || mayaiRulesFileInstructions) {
			// altering the system prompt mid-task will break the prompt cache, but in the grand scheme this will not change often so it's better to not pollute user messages with it the way we have to with <potentially relevant details>
			systemPrompt += addUserInstructions(settingsCustomInstructions, mayaiRulesFileInstructions, mayaiIgnoreInstructions)
		}
		...
		if (previousApiReqIndex >= 0) {
			...
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: mayaiApiReqInfo = JSON.parse(previousRequest.text)
				...
				if (totalTokens >= maxAllowedSize) {
					...
					await this.savemayaiMessages() // saves task history item which we use to keep track of conversation history deleted range
	...
	async recursivelyMakemayaiRequests(
		...
		if (this.consecutiveMistakeCount >= 3) {
			...
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "mayai uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.5 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
		...
		try {
			...
			try {
				...
				if (!this.abandoned) {
					...
					if (history) {
						await this.providerRef.deref()?.initmayaiWithHistoryItem(history.historyItem)
						// await this.providerRef.deref()?.postStateToWebview()
					}
				}
			} finally {
